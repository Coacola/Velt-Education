...
export function getPaymentStatusStyle(status: PaymentStatus) {
  const map = {
    paid:    { color: "text-green-400", bg: "bg-green-400/10", border: "border-green-400/20", label: "Εξοφλημένο", dot: "bg-green-400" },
    partial: { color: "text-amber-400", bg: "bg-amber-400/10", border: "border-amber-400/20", label: "Μερική Πληρωμή", dot: "bg-amber-400" },
    overdue: { color: "text-red-400",   bg: "bg-red-400/10",   border: "border-red-400/20",   label: "Ληξιπρόθεσμο", dot: "bg-red-400" },
  };
  return map[status];
}
...
export function getAttendanceStatusStyle(status: "present" | "absent" | "late" | "excused") {
  const map = {
    present: { color: "text-green-400", bg: "bg-green-400/10", label: "Παρών", dot: "bg-green-400" },
    absent:  { color: "text-red-400",   bg: "bg-red-400/10",   label: "Απών",  dot: "bg-red-400" },
    late:    { color: "text-amber-400", bg: "bg-amber-400/10", label: "Αργοπορία", dot: "bg-amber-400" },
    excused: { color: "text-white/40",  bg: "bg-white/5",      label: "Δικαιολογημένος", dot: "bg-white/30" },
  };
  return map[status];
}
